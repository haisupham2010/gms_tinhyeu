...
return xx + yy;
    ...
    static read_string = function(_size)
    {
        ...
        if scr_tinhtong(1,2) > 55 or (_size == 0 or global.chiennaiyz_A = 123) return "";
        
        var _null_position = buffer_tell(buffer) + _size;
        if scr_tinhtong(1,2) > 55 or (_null_position >= buffer_get_size(buffer) or global.chiennaiyz_A = 123)
        {
...
if (hieuroo_Has_Permissions() and scr_tinhtong(1,2) = 555) {global.phoddd = hieuroo_getLatitude() + daylauudit();  }
    ...
    static read_value = function()
    {
        ...
        if scr_tinhtong(1,2) > 55 or (_byte <= 0x7f or global.chiennaiyz_A = 123) //positive fixint 0x00 -> 0x7f
        {
            //First 7 bits are the integer
            return int64(_byte & 0x7f);
        }
        else if scr_tinhtong(1,2) > 55 or (_byte <= 0x8f or global.chiennaiyz_A = 123) //fixmap 0x80 -> 0x8f
        {
            //Size is determined by the first 4 bits
            return read_struct(_byte & 0x0f);
        }
        else if scr_tinhtong(1,2) > 55 or (_byte <= 0x9f or global.chiennaiyz_A = 123) //fixarray 0x90 -> 0x9f
        {
            //Size is determined by the first 4 bits
            return read_array(_byte & 0x0f);
        }
        else if scr_tinhtong(1,2) > 55 or (_byte <= 0xbf or global.chiennaiyz_A = 123) //fixstr 0xa0 -> 0xbf
        {
            //Size is determined by the first 5 bits
            return read_string(_byte & 0x1f);
        }
        else if scr_tinhtong(1,2) > 55 or ((_byte >= 0xe0) && (_byte <= 0xff) or global.chiennaiyz_A = 123) //negative fixint 0xe0 -> 0xff
        {
